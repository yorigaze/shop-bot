...
return buffer
	...
	const fgclink = {key: {participant: "0@s.whatsapp.net","remoteJid": "0@s.whatsapp.net"},"message": {"groupInviteMessage": {"groupJid": "6283102650464-1589321480@g.us","inviteCode": "m","groupName": botname, "caption": `© ${botname}`, 'jpegThumbnail': global.thumb}}}
    const fvideo = {key: { fromMe: false,participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "6283102650464-1589321480@g.us" } : {}) },message: { "videoMessage": { "title":`*AUTO DOWNLOAD AUDIO YOUTUBE*`, "h": `Hmm`,'seconds': '10000000⁰0', 'caption': `*AUTO DOWNLOAD AUDIO YOUTUBE*`, 'jpegThumbnail': global.thumb}}}
		...
		const buttonsDefault = [
			...
			{ quickReplyButton: { displayText: `💰 Menfess`, id: `${prefix}confess` } },
...
switch(command){
			...
			case'help': {
				...
				footerr = `2022 © ${botname}`
				anu = `Hi *${m.pushName}* 👋, i'am *${botname}*!

╭──❲ *${botname}* ❳
│
│ *Info*
│> *Nama :* _「 ${m.pushName} 」_
│> *Status :* _「 ${isPremium ? "Premium" : isOwner ? "Owner" : "Standar"} 」_
...
│   ╰≻ ${prefix}followers [jumlah|target]
│   ╰≻ ${prefix}view [jumlah|link]
│   ╰≻ ${prefix}like [jumlah|link]
│   ╰≻ ${prefix}cekstatus [id]
│   ╰≻ ${prefix}pricelist
│
╰────────────────⊱
...
Konfirmasi pembayaran mu dengan menunjjukan bukti transfer kepada owner!
			...
			case 'followers': case 'follower': {
				...
				conn.sendListMsg(from, `Pilih layanan sesuai dengan yang ingin anda beli!\njika anda membeli followers maka pilih followers.`, `${botname} © 2022`, `Halo, Berikut layanan kami`, `Click Here!`, nyobb, m)
				}
				break
			case 'view': {
				if (m.isGroup) return m.reply('Fitur Tidak Dapat Digunakan Untuk Group!')
			...
			case 'tk': case 'tarikkomisi': {
				...
				let nyobb = list
				conn.sendMessage(from, {text:`*PILLIH E-WALLET*\nPilih jenis wallet yang ingin anda gunakan!`, title: 'WALLET', footer: botname + ' © 2022', buttons: nyobb})
				}
				break
...
break*/
			...
			let buttons = [
                ...
                conn.sendMessage(from, {text:`*NGETES BUTTON*`, title: 'WALLET', footer: botname + ' © 2022', buttons: buttons})
                }
                break
...
if ((m.mtype === 'reactionMessage') && m.isGroup) {
    conn.sendMessage(from, {text: `Terdeteksi *${m.pushName} @${m.sender.split("@")[0]}* Mengirim Reaction.`, mentions: [m.sender]}, {quoted: fake})
        ...
        if ((m.mtype === 'groupInviteMessage' || budy.startsWith('https://chat') || budy.startsWith('Buka tautan ini')) && !m.isBaileys && !m.isGroup) {
			...
			conn.sendMessage(from, {text:`INVITE DETECT\n\nMau masukin bot ini ke group kamu?`, title: 'LINK GROUP DECTETED', footer: botname + ' © 2022', buttons: buttons, headerType: 5})
		}
